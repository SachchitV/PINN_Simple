# -*- coding: utf-8 -*-
"""
PyTorch version of trigonometric example
Created based on TensorFlow version from Mon Aug 17 22:42:20 2020
...
class ZeroOrderODE(PINNBaseModel):
    # Here we are trying to Approximate f(x) = y = x + sin(4*pi*x)
    ...
    def train_step(self, x):
        ...
        target = x + torch.sin(4 * np.pi * x)
        currentLoss = torch.mean((yHat - target)**2)
...
y_true = xTest + np.sin(4 * np.pi * xTest)
...
plt.xlabel('x')
plt.ylabel('y')
plt.title('Neural Network vs Actual Function: f(x) = x + sin(4πx)')
